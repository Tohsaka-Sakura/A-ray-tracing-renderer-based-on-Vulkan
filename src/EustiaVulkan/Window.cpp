#include "Window.h"
#include "Utillities/Exception.h"
#include "Utillities/StbImage.h"
#include <iostream>
...
namespace{
    ...
    {
        auto* const this_ = static_cast<Window*>(glfwGetWindowUserPointer(window));
        if(this_->OnCursorPostion)
        {
            this_->OnCursorPostion(xpos, ypos);
        }
    }
...
Window::Window(const WinodwConfig& config)
    :config_(config)
{
